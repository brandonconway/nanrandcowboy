...
function draw () {
  ...
  for (var i = 0, len = points.length; i < len; i++) {
    ...
    if (d < area) {
      ...
      ctx.moveTo( points[points.length-1].x + (dx * 0.22), points[points.length-1].y + (dy * 0.22));
      ctx.lineTo( points[i].x - (dx * 0.22), points[i].y - (dy * 0.22));
      ctx.stroke();
    }
  }
};
...
$(document).ready(function () {
  points.push({ x: curx, y: cury });
  var d = window.setInterval(function () { draw();}, 50);
});
